...
use all type Quote_Structure.Fields;
...
procedure Main is
   ...
   begin
      ...
      for Field in Fields loop
         ...
         begin
            ...
            Gtk_New (Text);
            Set_Property
              (Text, Gtk.Cell_Renderer_Text.Editable_Property, True);
            ...
            Set_Property
              (Column, Gtk.Tree_View_Column.Max_Width_Property,
               Geometry.Width / 3);
            --  Set_Property(
            --               Column,
            ...
            if Field = Quotation then
               Set_Property
                 (Text, Gtk.Cell_Renderer_Text.Wrap_Width_Property,
                  Geometry.Width / 3);
               Set_Property
                 (Text, Gtk.Cell_Renderer_Text.Wrap_Mode_Property,
   ...
   Home_Path   : String :=
     GNATCOLL.VFS.Get_Home_Directory.Display_Full_Name (Normalize => True) &
     Dir_Sep;
...
begin
   ...
   begin
      ...
      while not Tio.End_Of_File (Config_File) loop
         declare
            Line : String := Tio.Get_Line (Config_File);
         begin
      ...
      if Read_Result.Success then
         for Field in Fields loop
            if Read_Result.Value.Has_Field (To_String (Field_Names (Field)))
            then
               Column_Widths (Field) :=
   ...
   exception
      when E : Tio.Status_Error =>
         Tio.Put_Line
           ("** gtkadaquoter ** could not find configuration file " &
            Configuration_File);
      when others               =>
         Tio.Put_Line
           ("** gtkadaquoter ** could not read configuration file " &
            Configuration_File);
   end;
   ...
   begin
      if Read_Result.Success and then Read_Result.Value.Has_Field ("width")
      then
         Width := Read_Result.Value.Get ("width");
      end if;
      if Read_Result.Success and then Read_Result.Value.Has_Field ("height")
      then
         Height := Read_Result.Value.Get ("height");
      end if;
      IO.Put_Line (Width'Image & " x" & Height'Image);
   ...
   Save_Button.On_Button_Release_Event
     (Call => Save_Quotes_Cb'Access, Slot => List_Store, After => False);
   Save_Button.On_Mnemonic_Activate
     (Call  => Save_Quotes_Mnemonic_Cb'Access, Slot => List_Store,
      After => False);
   Add_Button.On_Button_Release_Event
     (Call => Add_Quote_Cb'Access, Slot => Tree_View, After => False);
   Add_Button.On_Mnemonic_Activate
     (Call => Add_Quote_Mnemonic_Cb'Access, Slot => Tree_View, After => False);
   Del_Button.On_Button_Release_Event
     (Call => Del_Quote_Cb'Access, Slot => Tree_View, After => False);
   Del_Button.On_Mnemonic_Activate
     (Call => Del_Quote_Mnemonic_Cb'Access, Slot => Tree_View, After => False);
   Quit_Button.On_Button_Release_Event
     (Call => Quit_Button_Cb'Access, Slot => Window_And_View, After => False);
   Quit_Button.On_Mnemonic_Activate
     (Call  => Quit_Button_Mnemonic_Cb'Access, Slot => Window_And_View,
      After => False);
   Win.On_Key_Release_Event
     (Call  => Window_Key_Release_Cb'Access, Slot => Window_And_View,
      After => False);

   -- Stop the Gtk process when closing the window
   Win.On_Delete_Event
     (Delete_Main_Window_Cb'Unrestricted_Access, Window_And_View);
   ...
   begin
      ...
      if Read_Result.Success and then Read_Result.Value.Has_Field ("corner_y")
      then
